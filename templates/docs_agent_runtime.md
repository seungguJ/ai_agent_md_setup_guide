...
이 문서에서 `Codex`는 관리자 역할 이름이다. 실제로 GPT가 관리자라면 동일한 규칙을 적용하고, 자동 호출 스크립트만 Claude용 런타임으로 이해하면 된다.
...
- 문서 관리자: `Codex` 또는 `GPT`
- 자동 구현 런타임: 현재 기본값은 `Claude`
- 다른 구현 Agent를 쓰면 이 문서와 스크립트를 해당 런타임 기준으로 다시 적는다
...
4. Claude 결과는 초안이며, 최종 승인과 사용자 응답은 Codex가 담당한다.
...
```bash
scripts/claude_handoff.sh --task-file docs/handoffs/p001.md
scripts/claude_handoff.sh --task-file docs/handoffs/p001.md --label review-pass-1
scripts/claude_handoff.sh --task-file docs/handoffs/p001.md --timeout-seconds 900
```

## 주의사항

- 자동 호출은 Claude가 실제 코드 수정까지 수행할 수 있는 강한 권한을 줄 수 있다.
- 불필요하게 넓은 작업 범위를 한 번에 넘기지 않는다.
